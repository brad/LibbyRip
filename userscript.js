...
`;
  function mainCode() {
    ...
    async function getDownloadZip() {
      ...
      if (window.__libregrabClientZipReady)
        return window.__libregrabClientZipReady;
...
`;
      ...
      for (let spine of BIF.objects.spool.components) {
        ret.push({
          url:
            location.origin +
            "/" +
            String(spine.meta.path).replace(/^\//, "") +
            "?" +
    ...
    function getMetadata() {
      ...
      let metadata = {
        ...
        coverUrl: BIF.root.querySelector("image")
    ...
    async function loadCover(metadata) {
      ...
      try {
        ...
        return {
          bytes,
          mime: blob.type || guessImageMime(metadata.coverUrl),
          name: "cover." + ext,
          blob,
        };
      } catch (e) {
    ...
    async function createMetadata() {
      ...
      const files = [];
      if (cover.blob)
        files.push({ name: "metadata/" + cover.name, input: cover.blob });
      files.push({
...
========================================= */
    ...
    function parseMpegHeader(u8, offset) {
      ...
      const isMpeg1 = ver === 3,
        isMpeg25 = ver === 0;
      const bitrate =
        (isMpeg1 ? BITRATE_MPEG1_L3 : BITRATE_MPEG2_L3)[brIdx] * 1000;
    ...
    function makeXingFrame(proto, frames, bytes) {
      ...
      if (off + 116 > frame.length)
        throw new Error("MPEG frame too small for Xing header");
      frame[off] = 0x58;
      frame[off + 1] = 0x69;
      frame[off + 2] = 0x6e;
      frame[off + 3] = 0x67;
    ...
    function chapterFramesForPart(metadata, spineIndex, durationSec) {
      ...
      for (let i = 1; i < spineChapters.length; i++) {
        if (spineChapters[i].offset <= spineChapters[i - 1].offset) {
          return {
            frames: [],
            count: 0,
            warning: "overlapping chapter offsets on spine " + spineIndex,
          };
        }
      }
      const frames = [];
      ...
      for (let i = 0; i < spineChapters.length; i++) {
        ...
        frames.push(
          chapFrame(
            cid,
            Math.round(last.offset * 1000),
            Math.max(
              Math.round(chap.offset * 1000) - 1,
              Math.round(last.offset * 1000),
            ),
            last.title,
          ),
      ...
      if (last !== null) {
        const spineDur =
          metadata.spine[last.spine] &&
          metadata.spine[last.spine].duration != null
            ? metadata.spine[last.spine].duration
            : durationSec;
        const endMs = Math.max(
          Math.round((spineDur || 0) * 1000),
          Math.round(last.offset * 1000),
        );
        childIds.push("last");
        frames.push(
          chapFrame("last", Math.round(last.offset * 1000), endMs, last.title),
        );
      }
      if (childIds.length)
        frames.unshift(ctocFrame("toc", childIds, "Table of Contents"));
    ...
    function absoluteBookChapters(metadata) {
      ...
      for (const ch of metadata.chapters || []) {
        ...
        const startMs = Math.round(
          ((spineOffsets[ch.spine] || 0) + (Number(ch.offset) || 0)) * 1000,
        );
        chapters.push({ title: ch.title, startMs });
      }
      for (let i = 0; i < chapters.length; i++) {
        chapters[i].endMs =
          i + 1 < chapters.length ? chapters[i + 1].startMs : totalMs;
      }
    ...
    function tagPartMp3(
      arrayBuffer,
      { partNumber, totalParts, durationSec, metadata, cover },
    ) {
      try {
        const author = getAuthorString();
        const narrator = getNarratorString();
        const title =
          (metadata && metadata.title) || BIF.map.title.main || "Untitled";
        const frames = [
          ...
          textFrame(
            "TRCK",
            totalParts ? partNumber + "/" + totalParts : String(partNumber),
          ),
        ...
        if (durationSec)
          frames.push(
            textFrame("TLEN", String(Math.round(durationSec * 1000))),
          );
        if (cover && cover.bytes && cover.bytes.length)
          frames.push(apicFrame(cover.bytes, cover.mime));
        const chap = chapterFramesForPart(
          metadata,
          partNumber - 1,
          durationSec,
        );
        ...
        return new Blob([writeId3(arrayBuffer, frames)], {
          type: "audio/mpeg",
        });
      } catch (e) {
    ...
    function buildWholeBookId3Tag(metadata, cover) {
      ...
      const desc =
        ...
        (metadata.description.full || metadata.description);
      if (desc) {
    ...
    async function createAndDownloadSingleMp3(urls, fileHandle) {
      const metadata = getMetadata();
      logLine(
        "Streaming single MP3 (one part in RAM). ffmpeg is not used.<br>",
      );
      const cover = await loadCover(metadata);
      const { tag, chapterCount, totalMs } = buildWholeBookId3Tag(
        metadata,
        cover,
      );
      logLine(
      ...
      async function fetchPartAudio(url) {
        ...
        if (!res.ok)
          throw new Error(
            "HTTP " + res.status + " for part " + (url.index + 1),
          );
      ...
      if (!proto)
        throw new Error(
          "part " +
            (urls[0].index + 1) +
            " did not start with a valid MPEG frame",
        );
        ...
        for (let i = 0; i < urls.length; i++) {
          ...
          if (!audio.length)
            throw new Error(
              "part " +
                (urls[i].index + 1) +
                " had no MPEG frames after header strip",
            );
...
const LIBREGRAB_PICKER_NS = "libregrab-picker-v1";
    ...
    function requestTopLevelMp3Handle(suggestedName) {
      ...
      if (typeof window.showSaveFilePicker !== "function") {
        alert(
          "Export as MP3 streams to disk and needs Chrome or Edge (File System Access API). Use Export audiobook for a zip instead.",
        );
        return;
      }
      let handle;
    ...
    async function createAndDownloadZip(urls, addMeta) {
      ...
      const fetchPromises = urls.map(async (url) => {
        ...
        if (!response.ok)
          throw new Error(
            "HTTP " + response.status + " for part " + (url.index + 1),
          );
        ...
        partElem.textContent =
          "Download of " +
          filename +
          " complete" +
          (chap.warning
            ? " (no CHAP: " + chap.warning + ")"
            : chap.count
              ? " (ID3 + " + chap.count + " chapters)"
              : " (ID3 tagged)");
      ...
      logLine(
        "<br><b>Downloads complete!</b> Starting ZIP generation and download...<br>",
      );
      ...
      if ("showSaveFilePicker" in window) {
        try {
          ...
          if (err.name === "AbortError")
            logLine("Download cancelled by user.<br>");
          else {
...
`;
    ...
    Function.prototype.bind = function (...args) {
      ...
      if (
        this.toString().includes("decryption") ||
        args.some(
          (arg) =>
            typeof arg === "function" && arg.toString().includes("decryption"),
        )
      ) {
        console.log("Decryption function detected", this);
        window.__libregrab_decryption_fn = args.find(
    ...
    function getBookComponents() {
      return BIF.objects.reader._.context.spine._.components.filter(
        (p) => !p.hidden && !(p.block && p.block.behavior),
      );
    }
    ...
    async function createContent(files, imgAssests) {
      ...
      downloadElem.innerHTML =
        "Gathering chapters <span id='chapAcc'>0/" + totComp + "</span><br>";
      downloadElem.scrollTo(0, downloadElem.scrollHeight);
      await waitForChapters(() => {
        downloadElem.querySelector("span#chapAcc").innerHTML =
          components.filter((page) => undefined != window.pages[page.id])
            .length +
          "/" +
          totComp;
      });
      ...
      logLine(
        "Downloading assets <span id='assetGath'>0/" +
          imgAssests.length +
          "</span><br>",
      );
      let gc = 0;
      await Promise.all(
        imgAssests.map(async function (name) {
          ...
          gc++;
          downloadElem.querySelector("span#assetGath").innerHTML =
            gc + "/" + imgAssests.length;
        }),
      );
    }
    ...
    function makePackage(files, assetRegistry) {
      ...
      const spine = doc.createElementNS(
        "http://www.idpf.org/2007/opf",
        "spine",
      );
      ...
      const ncxItem = doc.createElementNS(
        "http://www.idpf.org/2007/opf",
        "item",
      );
      ...
      assetRegistry.forEach((asset) => {
        ...
        let aname = asset.startsWith("http")
          ? getFilenameFromURL(asset)
          : asset;
    ...
    async function downloadEPUB() {
      ...
      files.push({
        ...
        input:
          '<?xml version="1.0" encoding="UTF-8"?><encryption xmlns="urn:oasis:names:tc:opendocument:xmlns:container"></encryption>',
      });
      ...
      logLine(
        "<br><b>Downloads complete!</b> Starting EPUB generation and download...<br>",
      );
    ...
    function bifFoundBook() {
      ...
      window.bif.cfc[1] = function (win, edata) {
        if (oldcrf1.boundArgs && oldcrf1.boundArgs[0]) {
          ...
          try {
            pages[win.name] = oldcrf1(win, edata);
          } catch (error) {
