...
export async function createAndDownloadMp3(
  BIF: BIF,
  ...
  buildAudiobookSingleMp3: (options: {
    urls: URLInfo[];
  ...
  if (metadata.coverUrl) {
    ...
    progress.add('Cover downloaded');
  }

  const filename = getAuthorString(BIF) + ' - ' + BIF.map.title.main + '.mp3';
  ...
  try {
    ...
    await buildAudiobookSingleMp3({
      urls: getUrls(BIF, odreadCmptParams),
      metadata,
      coverBytes,
      coverMime,
      fileHandle: handle,
      ...
      fetchWithRetry,
      BIF,
      getAuthorString: () => getAuthorString(BIF),
      getNarratorString: () => getNarratorString(BIF),
    });
    progress.clear();
    return;
  } catch (err) {
    ...
    if ((err as Error).name === 'LibreGrabSkipPicker') {
      // Expected: cross-origin player iframes cannot open a file picker.
    } else {
      console.error('Streaming download failed:', err);
    }
    progress.add('Streaming failed, using fallback...');
  }

  progress.add('Downloading and tagging chapters for ZIP...');
